...
| 7 | Other T (MAIT/γδ) | 341 | 2.3 | `Other T` label, TRAV/TRGV gene usage | Innate-like T cells bridging innate and adaptive arms |
...
Counts derive from the `adata.obs['leiden']` value counts, while dominant labels come from mapping each Leiden cluster to the majority `Cell.group` entry supplied with the CZI metadata.
...
- **CD4+ T cells (clusters 0, 10):** Helper T lymphocytes coordinating adaptive responses; central-memory signatures (CCR7/LTB) indicate surveillance and cytokine support for B cells and monocytes.
- **CD8+ T cells (clusters 1, 2, 9, 11):** Cytotoxic T lymphocytes mediating direct killing of infected cells via perforin/granzyme programs; cluster 9 is proliferating, while cluster 11 shows interferon-stimulated genes compatible with antiviral activation.
- **Other T (cluster 7):** Likely MAIT or γδ T cells based on `Other T` reference label—rapid responders residing in peripheral blood with innate-like cytokine secretion.
- **CD14+ Monocytes (clusters 3, 6):** Classical inflammatory monocytes capable of phagocytosis, antigen presentation, and cytokine release; the presence of both S100A8/9-rich and interferon-rich states suggests active recruitment.
- **B cells (cluster 4):** Mature B lymphocytes that present antigen and can differentiate into antibody-secreting plasmablasts.
- **Plasmablasts (cluster 5):** Short-lived, high-immunoglobulin secretory cells indicative of an active humoral response.
- **Platelets (cluster 8):** Anucleate fragments shed from megakaryocytes that contribute to coagulation and inflammatory mediator release; their transcript capture reflects platelet-rich PBMC preparations.
...
- **NK-cell expansion:** NK cells account for 1,883 cells (12.7%), also above healthy baselines (~5–10%) and consistent with innate antiviral activation.
- **Humoral activation:** Plasmablasts constitute 5.4% of cells (799 total), far exceeding the <1% seen in steady-state blood, indicating an ongoing antigenic challenge.
- **Inflammatory monocytes:** Classical CD14+ monocytes represent 13.5% of cells (1,995) and co-express S100A8/A9, pointing to systemic inflammation. A smaller CD16+ subset (129 cells) hints at progression toward patrolling phenotypes.
